...
namespace Sulakore.Habbo.Messages
{
    ...
    public class HMessage : IEquatable<HMessage>
    {
        ...
        public HMessage(ushort id, bool isOutgoing, string hash, string name, string structure, ASClass asClass)
            : this(id, isOutgoing, hash, name, structure)
        {
            References = new List<HReference>();

            Class = asClass;
            ClassName = asClass.QName.Name;
                    ...
                    case OPCode.CallProperty:
                        {
                            ...
                            if (callProperty.ArgCount > 0)
                            {
                                ...
                                foreach (var abc in HGame.ABCFiles)
                                {
        ...
        private string GetOutgoingStructure(ASClass @class)
        {
            ...
            {
                ASClass superClass = null;

                foreach (var abc in HGame.ABCFiles)
                {
                    superClass = abc.GetClass(@class.Instance.Super);
                ...
                for (i += 1; i < code.Count; i++)
                {
                    ...
                    if (previous.OP == OPCode.GetProperty)
                    {
                        ...
                        if (beforeGetProp.OP == OPCode.GetLex)
                        {
                            ...
                            foreach (var abc in HGame.ABCFiles)
                            {
                                classToCheck = abc.GetClass(getLex.TypeName);
